...
public class Utils {
	...
	public static Uri grayScaleFilter(Context context, Uri pathToImageFile) {
		...
		try (InputStream inputStream = new FileInputStream(
			...
			grayScaleImage = originalImage
					.copy(originalImage.getConfig(), true);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
		...
		return Utils.createDirectoryAndSaveFile(context, grayScaleImage,
	 ...
	 *            the web url.
	 * 
	 * @return the absolute path to the downloaded image file on the file
	 *         system.
	 */
	public static Uri downloadImage(Context context, Uri url) {
		try {
			...
			if (bitmap == null)
				...
				return Utils.createDirectoryAndSaveFile(context, bitmap,
						filename);
		} catch (Exception e) {
